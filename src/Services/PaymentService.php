...
public function allowedCountries(Basket $basket, $allowedCountry): bool
{
    ...
    */
    ...
    public function isGuaranteePaymentToBeDisplayed(Basket $basket, $paymentKey)
    {
        ...
        $billingAddressId = !empty($basket->customerInvoiceAddressId) ? $basket->customerInvoiceAddressId : $this->sessionStorage->getPlugin()->getValue('nnBillingAddressId');
            ...
            if(!is_null($basket) && $basket instanceof Basket && !empty($billingAddressId)) {
                ...
                if($this->settingsService->getPaymentSettingsValue('payment_active', $paymentKey) == true) {
                    ...
                    $value = $this->settingsService->getPaymentSettingsValue('minimum_order_amount', $paymentKey);
                    $configuredMinimumGuaranteedAmount = ((float) ($value ?? 0)) * 100;
                    ...
                    && ((int) $minimumGuaranteedAmount <= (int) $basketAmount)) {
     ...
     /**
     * Evaluate the Instalment payments conditions
     *
     * @param object $basket
     * @param string $paymentKey
     *
     * @return string
     */
    public function isInstalmentPaymentToBeDisplayed(Basket $basket, $paymentKey)
    {
        try {
        $billingAddressId = !empty($basket->customerInvoiceAddressId) ? $basket->customerInvoiceAddressId : $this->sessionStorage->getPlugin()->getValue('nnBillingAddressId');
            $shippingAddressId = !empty($basket->customerShippingAddressId) ? $basket->customerShippingAddressId : $this->sessionStorage->getPlugin()->getValue('nnShippingAddressId');
            if(!is_null($basket) && $basket instanceof Basket && !empty($billingAddressId)) {
                // Check if the instalment payment method is enabled
                if($this->settingsService->getPaymentSettingsValue('payment_active', $paymentKey) == true) {
                    // Get the customer billing and shipping details
                    $billingAddress = $this->paymentHelper->getCustomerAddress((int) $billingAddressId);
                    if(!empty($basket->customerShippingAddressId)) {
                    ...
                    // Get the billing and shipping details
                    $billingShippingDetails = $this->paymentHelper->getBillingShippingDetails($billingAddress, $shippingAddress);
                    // Set the minimum instalment amount
                    $configuredMinimumInstalmentAmount = $this->settingsService->getPaymentSettingsValue('minimum_order_amount', $paymentKey);
                    $minimumInstalmentAmount = (!empty($configuredMinimumInstalmentAmount) && $configuredMinimumInstalmentAmount >= 1999) ? $configuredMinimumInstalmentAmount : 1999;
                    // Get the basket total amount
                    $basketAmount = !empty($basket->basketAmount) ? $this->paymentHelper->convertAmountToSmallerUnit($basket->basketAmount) : $this->sessionStorage->getPlugin()->getValue('nnOrderAmount');
                    // First, we check the billing and shipping addresses are matched
                    // Second, we check the customer from the instalment payments supported countries
                    // Third, we check if the supported currency is selected
                    // Finally, we check if the minimum order amount configured to process the payment method. By default, the minimum order amount is 1999 cents
                    if($billingShippingDetails['billing'] == $billingShippingDetails['shipping']
                    && (in_array($billingShippingDetails['billing']['country_code'], ['AT', 'DE', 'CH']) || ($this->settingsService->getPaymentSettingsValue('allow_b2b_customer', $paymentKey)
                    && in_array($billingShippingDetails['billing']['country_code'], $this->getEuropeanRegionCountryCodes()) && $billingAddress->companyName ))
                    && (!empty($basket->currency) && $basket->currency == 'EUR')
                    && ((int) $minimumInstalmentAmount <= (int) $basketAmount)) {
                        // If the instalment conditions are met, display the instalment payments
                        return true;
                    }
                    // If none matches, error message displayed
                    return false;
                }
                // If payment instlament is not enabled, we hide that payment method
                return false;
            }
            // If payment instalment is not enabled, we hide that payment method
            return false;
        } catch(\Exception $e) {
            $this->getLogger(__METHOD__)->error('Novalnet::isInstalmentPaymentToBeDisplayedFailed', $e);
        }
    }
    ...
    */
    ...
    public function getBankDetailsInformation($transactionData)
    {
        if(in_array($transactionData['paymentName'], ['novalnet_instalment_invoice', 'novalnet_instalment_sepa'])) {
            $invoiceComments = PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('transfer_amount_duedate_text'), number_format($transactionData['cycle_amount'] / 100 ,2), $transactionData['currency'], date('d/m/Y', (int)strtotime($transactionData['due_date'])));
            ...
            $invoiceComments = PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('transfer_amount_duedate_text'), $transactionData['amount'], $transactionData['currency'], date('d/m/Y', (int)strtotime($transactionData['due_date'])));
            // If the transaction is in On-Hold not displaying the due date
            if($transactionData['tx_status'] == 'ON_HOLD') {
    ...
    /**
     * Form the instalment details comments
     *
     * @param array $transactionData
     *
     * @return string
    ...
    {
        $multibancoComments  = PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('multibanco_reference_text'), str_replace('.', ',', sprintf('%0.2f', $transactionData['amount'])), $transactionData['currency'] );
   ...
   public function getSavedPaymentDetails(&$paymentResponseData)
   {
       ...
       if(in_array($transactionData['paymentName'], ['novalnet_invoice', 'novalnet_guaranteed_invoice', 'novalnet_prepayment', 'novalnet_instalment_invoice'])) {
    ...
    public function doInstalmentVoid($transactionData, $paymentUrl)
    {
        try {
            ...
            $this->getLogger(__METHOD__)->error('Novalnet::doInstalmentVoid failed ' . $paymentRequestData['order_no'], $e);
        }
    }
